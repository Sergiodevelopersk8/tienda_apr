...
var index = 0;
var ringList = [];
var initX = (300 - spacing * (numItems - 1)) * 0.5;
...
function gotoIndex(targetIndex) {
  var distance = Math.abs(targetIndex - index) * spacing * 0.5;
  var duration = Math.min((distance / spacing) * 0.2, 0.4);
  TweenMax.to(dot, 0.15, {
    scaleX: 1.5,
    scaleY: 0.5,
  ...
  TweenMax.to(dot, duration, {
    delay: 0.175,
    fill: ringList[targetIndex].color,
    x: targetIndex * spacing,
    ease: Sine.easeInOut
  });
  TweenMax.to(dot, duration * 0.5, {
    delay: 0.175,
    y: -distance,
    ease: Sine.easeOut,
    yoyo: true,
